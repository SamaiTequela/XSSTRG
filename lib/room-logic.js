...
// A live draft older than this is treated as stale and not shown to the panel,
// so a speaker who closes their tab does not leave words on the jury's screen.
export const DRAFT_STALE_MS = 12000;
...
export function freshRoom(code, motion, perSecs, judgeMode) {
  ...
  return {
    ...
    // What the speaker holding the floor is typing right now, for the Crowd
    // Jury panel to follow. Never exposed to the opposing speaker.
    draft: null,
    verdict: null,
    verdictNotice: null,
    endedReason: null,
...
export function settleFlag(room) {
  // Prep is a deadline that only resolves when something reads it, and view()
  // resolves it on a throwaway copy without ever persisting. A room sitting in
  // storage therefore still carries an elapsed prepUntil and a null
  // turnStartedAt, so the flag check has to settle prep first or it decides
  // there is no running clock to flag.
  settlePrep(room);
...
export function view(room, clientId) {
  ...
  return {
    ...
    config: room.config,
    // The live draft is the panel's alone. Showing it to the opposing speaker
    // would hand them the other side's argument before it is delivered.
    draft: (isSpectator && room.draft && nowMs() - room.draft.at < DRAFT_STALE_MS)
      ? { side: room.draft.side, text: room.draft.text, wordCount: room.draft.wordCount }
      : null,
...
export function applyAction(room, action, clientId, body) {
  ...
  //
  // The one exception is the flagged speaker submitting their own words as
  // the flag falls. Settling first moves the floor off them and their turn
  // is then refused as "not your turn", losing whatever they had written --
  // the very speech the clock caught them mid-sentence on. Their own
  // submission takes precedence; the turn handler banks the spent clock and
  // closes the debate itself if both flags are down.
  const claimingOwnFlaggedTurn =
    (action === "turn" || action === "concede") && !!side && room.clock.active === side;
  const settled = claimingOwnFlaggedTurn ? false : settleFlag(room);
  ...
  switch (action) {
    ...
    case "turn": {
      ...
      if (outOfTime(room, side) && outOfTime(room, opp)) {
        ...
        room.turnNo += 1;
        beginPrep(room);
        // "I have said my piece, and I propose we stop there." Only the
        // speaker holding the stage may propose an end, so a player with
        // something still in the composer had to choose between putting it
        // on the record and asking to finish: submitting handed the stage
        // over, and their own request was then refused. The proposal now
        // rides along with the speech. The opponent still has to accept it.
        if (body.requestEnd) {
          room.endRequest = { from: side, at: nowMs() };
        }
      }
      room.draft = null; // the speech is in the record now; stop streaming it
      touch();
      return { mutated: true };
    }

    // The speaker holding the floor streams their draft to the Crowd Jury.
    // This used to travel over a BroadcastChannel, which only reaches other
    // tabs on the same machine -- so a panel on their own devices, which is
    // the whole point of an online jury, saw nothing at all.
    case "draft": {
      if (room.phase !== "debate") return { mutated: false };
      if (!side || side !== room.clock.active) return { mutated: false };
      if (!room.config.judgeMode) return { mutated: false };
      const text = clip(body.text, MAX_TEXT);
      const words = text.trim() ? text.trim().split(/\s+/).filter(Boolean).length : 0;
      room.draft = { side, text, wordCount: words, at: nowMs() };
      // Deliberately no touch(): a draft is not room activity worth extending
      // the room's life over, and it must not disturb the poll's backoff.
      return { mutated: true };
    }
